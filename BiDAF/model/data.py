...
class SQuAD():
    def __init__(self, args, path):
        ...
        test_examples_path = os.path.join(dataset_path, 'test_examples.pt')
        ...
        self.dict_fields = {'id': ('id', self.RAW),
                       's_idx': ('s_idx', self.LABEL),
                       'e_idx': ('e_idx', self.LABEL),
                       'context': [('c_word', self.WORD), ('c_char', self.CHAR)],
                       'question': [('q_word', self.WORD), ('q_char', self.CHAR)]}

        self.list_fields = [('id', self.RAW), ('s_idx', self.LABEL), ('e_idx', self.LABEL),
                       ('c_word', self.WORD), ('c_char', self.CHAR),
                       ('q_word', self.WORD), ('q_char', self.CHAR)]

        if os.path.exists(dataset_path):
            # if os.path.exists(f'{dataset_path}{test_examples_path}'):
            #     print("loading test dataset...")
            #     test_examples = torch.load(test_examples_path)
            #     self.test = data.Dataset(examples=test_examples, fields=self.list_fields)
            # else:
            print("building testing dataset...")
            self.test = data.TabularDataset(path=f'{path}/{args.test_file}l', format='json', fields=self.dict_fields)
            torch.save(self.test.examples, test_examples_path)

            print("loading train & dev dataset...")
            train_examples = torch.load(train_examples_path)
            dev_examples = torch.load(dev_examples_path)
            self.train = data.Dataset(examples=train_examples, fields=self.list_fields)
            self.dev = data.Dataset(examples=dev_examples, fields=self.list_fields)
        ...
        self.WORD.build_vocab(self.train, self.dev, vectors=GloVe(dim=args.word_dim))
